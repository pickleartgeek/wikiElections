...
/**
 * Turn weighted polling averages into per-candidate alpha (shape)
 * parameters. alpha_i = concentration * avgShare_i, where
 * `concentration` scales with the total effective polling weight so
 * more/better polling narrows the distribution (higher alpha =
 * tighter, more confident simulation). beta is fixed at 1 throughout.
 */
function PC_computeAlphas(polls, candidateIds, opts = {}) {
  const agg = BaseCalc_aggregate(polls, candidateIds);
  const concentration = opts.concentration || Math.max(4, agg.totalWeight * 1.5);
  const alphas = {};
  candidateIds.forEach(id => {
    // floor so a candidate polling at 0% still has a nonzero (tiny) shot
    alphas[id] = Math.max(0.05, agg.averages[id] * concentration);
  });
  return { alphas, averages: agg.averages, concentration, totalWeight: agg.totalWeight };
}
 ...
 */
function ProbCalc_run(polls, candidateIds, simulations = 20000, opts = {}) {
  const { alphas, averages, concentration, totalWeight } = PC_computeAlphas(polls, candidateIds, opts);
  ...
  return { winProb, meanShare, alphas, pollAverages: averages, concentration, totalWeight, simulations };
}
