...
function RCPDonut_render(container, opts) {
  ...
  // strokeWidth has to be known before r, since r needs to leave room for
  // half the ring's own stroke width inside the viewBox -- the old code
  // computed r first from a flat magic-number margin (18px) that didn't
  // scale with the stroke, so on the default 260px size the ring's outer
  // edge (r + strokeWidth/2) actually exceeded the viewBox half-width and
  // got clipped by the SVG's own bounds.
  const strokeWidth = Math.max(28, size * 0.16);
  const ringPad = 4; // small breathing-room margin beyond the stroke itself
  const r = size / 2 - strokeWidth / 2 - ringPad;
